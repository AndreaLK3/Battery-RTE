...
def exe():
    ...
    logging.info("Average over "+ str(len(trips)) + " roundtrip segments = " + str(round(sum(rte_ls)/ len(rte_ls)*100, 2)) + "%")
    ...
    logging.info("Average over "+ str(len(trips)) + " roundtrip segments (weighted by each trip's Δ SOC) = " + str(round(weighted_avg*100, 2)) + "%")
