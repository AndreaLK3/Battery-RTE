...
def exe():
    ...
    logging.info("Average RTE over "+ str(len(trips)) + " trips = " + str(round(sum(rte_ls)/ len(rte_ls)*100, 2)) + "%")
    ...
    logging.info("Average RTE over "+ str(len(trips)) + " trips (weighted by each trip's Δ SOC) = " + str(round(weighted_avg*100, 2)) + "%")
